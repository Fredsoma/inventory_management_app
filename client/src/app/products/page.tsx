...
type ProductFormData = {
  ...
  rating: number;
  imageUrl: string;  // Define the type as string
};

// Define an array of image URLs with unique tokens (just an example, we will use the Pinata URL later)
const imageUrls = [
          ...
          products?.map((product) => (
            ...
            >
              <div className="flex flex-col items-center">
                {/* Display the product image using the image URL from Pinata */}
                <Image
                  src={product.imageUrl || imageUrls[0]} // Use the imageUrl from Pinata or fallback to a random one
                  alt={product.name}
                  width={150}
                  height={150}
